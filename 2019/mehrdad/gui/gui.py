...
class Contact:
    ...
    def fill_table(self, tree):
        ...
        for item in enumerate(data):
            tree.insert('', 'end', iid=item[1][0], values=(item[0]+1,) + item[1])
...
class ContactApp(tk.Tk):
    def __init__(self):
        ...
        self.contact_table['columns'] = ('row', 'id', 'first_name', 'last_name', 'phone_number')
        self.contact_table.heading('row', text='Row')
        ...
        self.contact_table['displaycolumns'] = ('row', 'first_name', 'last_name', 'phone_number')

        # action frame
        add_button = ttk.Button(self.action_frame, text='Add', command=self.add_contact_form)
        edit_button = ttk.Button(self.action_frame, text='Edit')
        ...
        edit_button.config(command = self.update_contact_form)
    ...
    def update_contact_form(self):
        ...
        update_button = ttk.Button(update_form, text='Update')
        ...
        phone_number.insert(0, values[4])
        # update button will call a function to update on db
        
        update_button.config(command=lambda: self.action.update_contact(
            {
